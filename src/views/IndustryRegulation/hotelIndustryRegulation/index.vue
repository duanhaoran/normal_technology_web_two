      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            ...
            circle
            size="small"
            @click="ViewModel(scope.row.enterpriseId)"
...
<!--            circle-->
<!--            size="small"-->
<!--            @click="UpdateModel(scope.row.enterpriseId)"-->
<!--            class="staff-table-opera-btn">-->
<!--          </el-button>-->
<!--          &lt;!&ndash;删除详情按钮&ndash;&gt;-->
          <el-button
            ...
            circle
            size="small"
            @click="DeleteModel(scope.row.enterpriseId)"
            class="staff-table-opera-btn">
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog title="信息修改" :visible.sync="updateVisible" center width="40%" top="6vh">
      ...
      <el-form :model="InfoModel" label-width="80px" label-position="left" >
        ...
        <el-form-item label="企业名称" prop="enterpriseName">
          <el-col :span="12">
            <el-input v-model="InfoModel.enterpriseName" clearable style="width: 300px" />
          </el-col>
        </el-form-item>
        <el-form-item label="企业状态" prop="enterpriseStats">
          <el-col :span="12">
            <el-input v-model="InfoModel.enterpriseStats" clearable style="width: 300px" />
          </el-col>
        </el-form-item>
        <!--信息修改按钮-->
        <el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        enterpriseStats: '',
        //url
        selectByAny: '/api/enterprise/selectEnterpriseByAny',
        selectOne: '/api/enterprise/selectEnterpriseOne',
        updateOne: '/api/enterprise/updateEnterpriseOne',
        insertOne: '/api/industryEnterprise/insertEnterpriseIndutryOne',
        deleteByList: '/api/industryEnterprise/deleteEnterpriseindByList',
        deleteOne: '/api/industryEnterprise/deleteIndustryEnterpriseOne',
        selectNotInEnterprise: '/api/enterprise/selectNotInEnterprise',
      }
    },
    created() {
      //面包屑创建方法
      ...
      this.$axios.get(
        this.selectByAny,
        {pageNum: this.pageNum, pageSize: this.pageSize,industryId:this.industryId},
        (res) => {
          if (res.resultCode === 1) {
    ...
    methods: {
      ...
      SelectByAny() {
        this.$axios.get(
          this.selectByAny,
          {
            ...
            enterpriseName: this.SelectModel.enterpriseName,
      ...
      InsertWithModel() { // 查看学生详情
        ...
        this.$axios.get(
          this.selectNotInEnterprise,
          {
            industryId: this.industryId
          },
          (res) => {
            if (res.resultCode === 1) {
              this.resultListIns = res.date.dataList
            }
          }
        );
      },
      ...
      DeleteByList() {
        if (this.multipleSelection.length !== 0) {
          this.$confirm('是否删除信息？', '警告', {
            ...
            this.$axios.del(
              this.deleteByList,
              {ids: listuserId,industryId: this.industryId},
              (res) => {
                if (res.resultCode === 1) {
                  this.successMession();
                  this.SelectByAny()
      ...
      DeleteModel(id) {
        this.$confirm('是否删除信息？', '警告', {
          ...
          this.$axios.del(
            this.deleteOne,
            {enterpriseId: id,industryId:this.industryId},
            (res) => {
              if (res.resultCode === 1) {
                this.successMession();
                this.SelectByAny()
